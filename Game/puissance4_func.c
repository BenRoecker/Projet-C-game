...
Plateau* creer_plateau(int lignes, int colonnes)
{
    ...
    //On crée un tableau dynamique 2D de char
    ...
    for(int i = 0; i < lignes; i++)
    {
        ...
        {
            plateau -> grille[i][n] = '.'; //On l'initialise avec des '.' dedans
        }
    }
    return plateau;
}
...
void detruire_plateau(Plateau** adr_plateau)
{
    ...
    //On détruit le tableau 2D
    for(int i = 0; i < plateau->lignes; i++)
    {
        free(plateau->grille[i]);
    }
    //On détruit la structure ; les éléments statiques sont détruits seuls
    free(plateau);
    *adr_plateau = NULL;
}
